"""
Risk Spoke - MCP Server for Risk Management Tools
"""
import asyncio
import json
import sys
...
# Tool registry
TOOLS_REGISTRY = {
    "risk.detect_anomaly": anomaly_detector,
    "risk.check_compliance": compliance_checker
}
...
async def register_with_hub():
    """Register this Risk Spoke with the Hub"""
    try:
...
async def lifespan(app: FastAPI):
    ...
    print("Risk Spoke MCP Server started successfully")

    yield

    # Shutdown
    print("Shutting down Risk Spoke MCP Server...")

# FastAPI application
app = FastAPI(
    title="Risk Spoke MCP Server",
    description="MCP Server for Risk Management Tools",
    version="1.0.0",
    lifespan=lifespan
)
